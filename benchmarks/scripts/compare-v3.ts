...
console.log('Dataset                    JSON     TOON   GCF-v2  GCF-v3a    PLOON  v3vsv2 v3vsPLOON  v3vsTOON')
...
for (const ds of TOKEN_EFFICIENCY_DATASETS) {
  ...
  const gcfV2Tokens = tokenize(formatters['gcf'](ds.data))
  ...
  const v3vsToon = ((1 - gcfV3Tokens / toonTokens) * 100).toFixed(1)
  
  const name = ds.name.padEnd(26)
  console.log(`${name} ${jsonTokens.toString().padStart(7)}  ${toonTokens.toString().padStart(7)}  ${gcfV2Tokens.toString().padStart(7)}  ${gcfV3Tokens.toString().padStart(7)}  ${ploonTokens.toString().padStart(7)}  ${(v3vsV2 + '%').padStart(6)} ${(v3vsPloon + '%').padStart(9)} ${(v3vsToon + '%').padStart(8)}`)
}
...
console.log(`${'TOTAL'.padEnd(26)} ${totals.json.toString().padStart(7)}  ${totals.toon.toString().padStart(7)}  ${totals.v2.toString().padStart(7)}  ${totals.v3.toString().padStart(7)}  ${totals.ploon.toString().padStart(7)}  ${(v3vsV2T + '%').padStart(6)} ${(v3vsPloonT + '%').padStart(9)} ${(v3vsToonT + '%').padStart(8)}`)
console.log(`\nGCF v3 vs JSON: ${v3vsJsonT}% fewer tokens`)
